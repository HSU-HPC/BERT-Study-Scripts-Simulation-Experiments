...
findbestparams <- function(){
    ...
    classes <- 2
    mvfravtion <- 0.1
    corered <- c(2,4,8,16,32,64)
    stopParB <- c(2,4,8,16,32,64,128)
    ...
    result_path = "results/params_strong_big.csv"
        ...
        for(back in backends){
            ...
            for(stop in stopParB){
                for(red in corered){
                    ...
                    total_start <- Sys.time()
                    dataset_adjusted <- BERT(dataset, 64, method="ComBat", verify=FALSE, qualitycontrol=FALSE, backend = back, stopParBatches=stop, corereduction=red)
                    total_end <- Sys.time()
                    execution_time <- as.numeric(as.POSIXct(total_end,origin = "1970-01-01")) - as.numeric(as.POSIXct(total_start,origin = "1970-01-01"))
                    result_df[nrow(result_df) + 1,] = c(rep, 64, execution_time, back, stop,red)
                    gc()
                }
                # write to file
...
result_path = "results/Results_strong_big.csv"
